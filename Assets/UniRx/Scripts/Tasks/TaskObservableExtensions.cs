...
#else // using Parse Implementation of Task: https://github.com/ParsePlatform/Parse-SDK-dotNET/tree/master/Unity.Tasks/Public
...
namespace UniRx
{
    ...
    public static class TaskObservableExtensions
    {
        // and needs 'this Task task' overload.
        public static IObservable<T> ToObservable<T>(this Task<T> task)
        {
            if (task.IsFaulted)
            {
                return Observable.Throw<T>(task.Exception);
            }
            ...
            if (task.IsCanceled)
            {
                return Observable.Throw<T>(new OperationCanceledException()); // was TaskCanceledException(task)
            }
